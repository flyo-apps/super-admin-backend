...
from ..models.stories import (
    StoryCreateBaseModel,
    StoryCreateModel,
    StoryUpdateBaseModel,
    StoryUpdateModel,
    StoryDeleteModel
)
...
class StoriesCollection:
    def __init__(self):
        self.model = CRUDBase(StoriesSchema)

    async def create_story(
        self,
        story_details: StoryCreateBaseModel,
        db: Session
    ) -> any:
        try:
            ...
            story_create = StoryCreateModel(**story_details.dict())
    ...
    async def update_story(
        self,
        story_update_details: StoryUpdateBaseModel,
        db: Session
    ) -> any:
        try:
            ...
            story_update = StoryUpdateModel(**story_update_details.dict(exclude_unset=True))
        ...
        try:
            ...
            for story in story_details:
                if story.code not in existing_codes:
                    story_create = StoryCreateModel(**story.dict())
                    stories_create.append(story_create.dict())
                else:
                    story_update = StoryUpdateModel(**story.dict())
                    story_update.is_updated = True
                    story_update.updated_at = datetime.now()
    ...
    async def delete_story_by_code(
        ...
        try:
            ...
            story_delete_dict = StoryDeleteModel(code=story_code).dict()
