...
from items.models.quick_filters import QuickFilterCreateModel, QuickFilterUpdateModel

from sqlalchemy.orm import Session
from db.redis.redis_base import RedisBase
from ..schemas.quick_filters import QuickFiltersSchema
from db.aurora.aurora_base import CRUDBase
import json
import uuid
from ..utils.constants import QUICK_FILTER_MAIN_KEY

class QuickFiltersCollection:
    def __init__(self):
        self.model = CRUDBase(QuickFiltersSchema)
    ...
    async def create_quick_filters(
        ...
        try:
            ...
            quick_filter_create = QuickFilterCreateModel(code=str(uuid.uuid4()), screen_filter=json.loads(json.dumps(screen_filter).lower()), data=data)
            created_quick_filter = self.model.create(db=db, obj_in=quick_filter_create)

            if created_quick_filter:
                await self.redis.hset_list_dict(main_key=QUICK_FILTER_MAIN_KEY,sub_key=json.loads(json.dumps(screen_filter).lower()), list_data=data)
    ...
    async def update_quick_filters(
        ...
        try:
            ...
            quick_filter_update = QuickFilterUpdateModel(data=data)
    ...
    async def get_quick_filters(
        ...
        try:
            quick_filter = await self.redis.hget_list_dict(main_key=QUICK_FILTER_MAIN_KEY, sub_key=json.loads(json.dumps(screen_filter).lower()))
            ...
            await self.redis.hset_list_dict(main_key=QUICK_FILTER_MAIN_KEY,sub_key=json.loads(json.dumps(screen_filter).lower()), list_data=existing_quick_filter.__dict__["data"])
    ...
    async def delete_quick_filters(
        ...
        try:
            ...
            if updated_quick_filter == None:
                await self.redis.hdel_list_dict(main_key=QUICK_FILTER_MAIN_KEY,sub_key=json.loads(json.dumps(screen_filter).lower()))
